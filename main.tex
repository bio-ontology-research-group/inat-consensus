...
Here is the clean, rewritten Results: Instantiation (ABox) section without any citation markers, ready for compilation.
Code snippet

\section{Results: Instantiation (ABox)}

The ABox contains the assertions derived from the Rub' al Khali project on iNaturalist (\url{https://www.inaturalist.org/projects/rub-al-khali}).
...
\subsection{Example Instance: The Evolution of Consensus}
The following Turtle snippet demonstrates how we capture the history
of a revision: an agent initially identifying a specimen as a generic
genus, and later refining it to a species that conflicts with another
expert. The example below explicitly models the temporal sequence of
events ($T_1 \rightarrow T_2 \rightarrow T_3$) and the resulting shift
in consensus state.

\begin{lstlisting}[language=SPARQL, caption=ABox instance showing revision and conflict]
@prefix sio: <http://semanticscience.org/resource/> .
@prefix ex: <http://example.org/rub-al-khali/> .
@prefix xsd: <http://www.w3.org/2001/XMLSchema#> .

# The Observation (Biological Reality)
ex:obs_1942 a sio:observation ;
    rdfs:label "Desert Shrub Observation" .

# --- TIME T1: Agent A identifies as Genus Cornulaca ---
ex:id_act_1 a ex:IdentificationAct ;
    sio:has-agent ex:agent_A ;
    sio:has-target ex:obs_1942 ;
    sio:has-output ex:taxon_Cornulaca ; # Genus level
    sio:has-value "2025-01-10T10:00:00"^^xsd:dateTime .

# --- TIME T2: Agent B identifies as Species C. monacantha ---
ex:id_act_2 a ex:IdentificationAct ;
    sio:has-agent ex:agent_B ;
    sio:has-target ex:obs_1942 ;
    sio:has-output ex:taxon_Cornulaca_monacantha ;
    sio:has-value "2025-01-11T14:30:00"^^xsd:dateTime .
    # Note: Compatible with Act 1 (Species is sub-class of Genus)

# --- TIME T3: Agent A Revises to Species C. arabica ---
ex:id_act_3 a ex:IdentificationAct ;
    sio:has-agent ex:agent_A ;
    sio:has-target ex:obs_1942 ;
    sio:has-output ex:taxon_Cornulaca_arabica ;
    sio:has-value "2025-01-12T09:15:00"^^xsd:dateTime ;
    sio:is-successor-of ex:id_act_1 . 
    # Logic: Act 1 is superseded. Conflict is now between Act 2 and Act 3.

# --- INFERENCE RESULT ---
# Since C. arabica and C. monacantha are disjoint:
# ex:obs_1942 a ex:ConflictingObservation .
\end{lstlisting}

This modeling approach ensures that while the ABox contains contradictory evidence (Act 2 vs. Act 3), the logical integrity of the Observation entity is preserved, and the conflict is flagged for review.
